...
public class PrintJob : Object {
    ...
    public async File? run(ProgressDialog? progress_dialog, Cancellable? cancellable) throws Error {
    ...
    private Error? printing_thread() {
        ...
        try {
            ...
            tmp_file = File.new_tmp("shotwell-printing-XXXXXX", out iostream);
            output_stream = iostream.get_output_stream();
            ...
            iostream.close(this.cancellable);
        } catch (Error err) {
            return err;
        } finally {
    ...
    private Cairo.Status pdf_write(uchar[] data) {
        try {
            ...
            warning("PDF surface write error: %s", error.message);
